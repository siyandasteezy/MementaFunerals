...
export default function DashboardPage() {
  ...
  function loadData() {
    ...
    setUser(u);
    if (u) {
      const all = getAllPrograms().filter((p) => p.userId === u.id);
      setPrograms(all);
    }
  }
        ...
        <main className="flex-1 p-8 overflow-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-500 mt-1">
              Manage your funeral programs and share them with loved ones.
            </p>
          </div>
          ...
          {programs.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-16 text-center">
              ...
              <p className="text-gray-400 text-sm mb-6">
                Create your first digital funeral program and share it with a QR code.
              </p>
              <Link
                href="/create"
                className="inline-flex items-center gap-2 bg-[#0F2B5B] hover:bg-[#1a3d7c] text-white px-6 py-3 rounded-lg text-sm font-medium transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
