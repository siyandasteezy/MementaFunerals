...
import { getProgram, getPDF, updateProgramViews } from '@/lib/storage';
    ...
    async function load() {
      try {
        ...
        if (blob) {
          ...
          setError('PDF not available. Please contact the organizer for more information.');
        }
      } catch {
    ...
    return (
      <div className="min-h-screen bg-[#0F2B5B] flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <Link
            href="/"
            className="inline-block bg-[#C49A22] hover:bg-[#B8860B] text-white px-6 py-3 rounded-xl text-sm font-semibold transition-colors"
          >
            Go to Mementa
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="bg-gradient-to-br from-[#0F2B5B] via-[#1a3d7c] to-[#0F2B5B] text-white py-10 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-blue-300 text-xl font-light">
            {program.birthYear} &mdash; {program.deathYear}
          </p>
          {eventDate && (
        ...
        {blobUrl ? (
          ...
          <div className="bg-white rounded-2xl shadow-md p-12 text-center border border-gray-200">
            ...
            <p className="text-gray-500 text-sm">
              The PDF is not available on this device. Please view on the original device or contact the organizer.
            </p>
          </div>
        )}
      </div>
      ...
      <footer className="bg-[#0F2B5B] text-blue-200 py-6 px-4 text-center text-xs">
        <p>
          Shared via{' '}
          <Link href="/" className="text-[#C49A22] hover:text-[#B8860B] font-semibold">
            Mementa
          </Link>{' '}
